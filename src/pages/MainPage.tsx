...
const MainPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchModes = async () => {
      ...
      try {
        ...
        if (dbMode) {
          // DB에서 가져온 모드 데이터가 있으면 사용
          const apiResponse: ApiResponse = dbMode;
          setModes(apiResponse.mode);

          // 마지막 모드를 현재 모드로 설정
          if (apiResponse.mode.length > 0) {
            setMode(apiResponse.mode[apiResponse.mode.length - 1].mode);
          }
        } else {
          // DB에 데이터가 없으면 기본값 설정
          setModes([
            { date: new Date().toISOString().split("T")[0], mode: "safe" },
          ]);
          setMode("safe");
        }
      } catch (err) {
  ...
  const handleSeedUpdate = (newSeed: number) => {
    console.log("MainPage에서 시드 업데이트:", newSeed);
