...
interface TradeCalculatorProps {
  ...
  zeroDayTrades?: Trade[];
  onSeedUpdate?: (seed: number) => void; // 추가: 시드 업데이트 콜백
}
...
const TradeCalculator: React.FC<TradeCalculatorProps> = ({
  ...
  // 시드 값이 변경될 때 콜백 호출
  useEffect(() => {
    if (onSeedUpdate && latestSeed !== settings.currentInvestment) {
      onSeedUpdate(latestSeed);
    }
  }, [latestSeed, onSeedUpdate, settings.currentInvestment]);
  ...
  useEffect(() => {
    ...
    fetchLatestSeed();
  }, [userId, settings.currentInvestment]);

  // settings.currentInvestment가 변경될 때도 latestSeed 업데이트
  useEffect(() => {
    if (settings.currentInvestment !== latestSeed) {
      setLatestSeed(settings.currentInvestment);
    }
  }, [settings.currentInvestment]);
